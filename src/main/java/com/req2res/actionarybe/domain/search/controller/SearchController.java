...
public class SearchController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    - 인증: permitAll (Authorization 헤더는 선택)
                    - 로그인 상태라면 isMine(내가 쓴 글 여부)를 계산하여 내려줍니다.
                """
    )
    @ApiResponses({
            @ApiResponse(
    ...
    @GetMapping("/posts")
    public ResponseEntity<Response<PostSearchPageResponseDTO>> searchPosts(
            @AuthenticationPrincipal UserDetails userDetails,

            @Parameter(description = "검색어(필수)", example = "인증")
            @RequestParam(name = "q") String q,
        ...
        PostSearchPageResponseDTO data =
                searchService.searchPosts(q, sort, page, size, userId);

        return ResponseEntity.ok(Response.success("게시글 검색 결과입니다.", data));
    }
